...
class Settings(BaseSettings):
    ...
    # Dry-run: вся логика решений (watcher -> симуляция/фильтр -> сайзинг ->
    # риск-проверки -> логика входа/выхода) выполняется как обычно, но
    # реальная отправка на блокчейн (send_raw_transaction / bundle.send())
    # пропускается — используется для проверки нового профиля/стратегии на
    # проде без риска капитала, см. README "Dry-run режим".
    dry_run: bool = False
    ...
    # Леджер ЗАКРЫТЫХ сделок (common/pnl_ledger.py) — в отличие от
    # trade_log_file (попытка = одна запись, entry/exit копитрейдинга/
    # снайпинга не связаны друг с другом), здесь одна запись = одна
    # реально закрытая позиция с посчитанным realized_pnl. Пишется только
    # при include=True — та же "честный факт, не оценка" логика.
    pnl_ledger_file: str = "pnl_ledger.jsonl"
